package com.dandelion.security;
	...
	public static class FormLoginWebSecurityConfigurerAdapter extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			http.authorizeRequests()
					.antMatchers("/api/create", "/", "controller/download","/assets/**", "/plugins/**", 
							"/static/**", "/bootstrap/**","/v2/api-docs/**", "/swagger-ui.html**",
							"/webjars/**", "/swagger-resources/**", "/api/**").permitAll() // 免认证目录
					.antMatchers("/admin/**").hasRole("ADMIN")// ADMIN角色可以访问/admin目录
					.and().formLogin().loginPage("/login")// 自定义登录页为/login
